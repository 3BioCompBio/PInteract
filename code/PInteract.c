...
/*---------------------------*/
{
    ...
    *nres=0;
    ...
    while(fgets(ligne,2560,seqin)!=NULL)
    {
        ...
        if(strncmp(ligne,"ENDMDL",6)==0)break;
        strncpy(nomres,&ligne[17],3);
        cr[0]=lett(nomres);
        if(strchr(residus,cr[0])==NULL)continue;
        if((strcmp(mot,"ATOM")==0||strncmp(mot,"HETATM",6)==0)&&strncmp(nomres,"HOH",3)!=0&&strncmp(no,noold,6)!=0)
        {
            if(cr[0]=='x')
                (*nres)+=5;
            else
...
/*-------------------------------------------*/
{
    ...
    int aro,doub0,doubleval0,doub,doubleval;
    int comp,i1,i2;
    int deja;
    int *fldeja,*fl,*repeat,memcount,memcountold;
    int num[5][5],fou,q,q0,rep;
    ...
    float d,dm,d1,d2,tt[3],ttN[3],ttN1[3];
    float dc5,dc6,ff;
...
//        }
        ...
        for(i=0;i<nres;i++)
        {
            ...
            for(j=0;j<nres;j++)
            {
                ...
                {
                    for(iiq=0;iiq<iimax1+doubleval;iiq++)
                    {
                        if(iiq<iimax1)dm=dist2(coordtot[i][iip],coordtot[j][iiq]);
                        else dm=dist2(coordtot[i][iip],coord[j][iiq-iimax1]);
                        if(dm<d)
                        {
                            d=dm;
                            iim=iip;
                ...
                if(!doub)q=0;
                else if(iima>=iimax1)q=iima-iimax1;
                else
                {
                ...
                if(d<=(CATDmax*CATDmax))
                {
                    ...
                    if(aro&&iima>=iimax1)anglemaxaro(d,&amax,coordtot[j][4],coord[j][q],coordN[j][q],CATradcyl);
                    else if(aro)anglemaxaro(d,&amax,coordtot[j][iima],coord[j][q],coordN[j][q],CATradcyl);
                    ...
                    if(a<=amax)
                    {
                        ...
                        if(flagADN[j]=='0')
                        {
                            ...
                            if(iima>=iimax1&&seq[j]=='H')
                                strcpy(aux4,"mC5");
                            else if(iima>=iimax1&&q==0)
                                strcpy(aux4,"mC6");
                            else if(iima>=iimax1&&q==1)
                                strcpy(aux4,"mC5");
                            else
        ...
        for(i=0;i<nres;i++)
        {
            ...
            for(j=0;j<nres;j++)
            {
                ...
                if(doub0)doubleval0=2;
                else doubleval0=1;
                if(doub)doubleval=2;
                else doubleval=1;

                d=(float)FLT_MAX;
                for(iip=0;iip<iimax+doubleval0;iip++)
                {
                    for(iiq=0;iiq<iimax1+doubleval;iiq++)
                    {
                         if(iip<iimax&&iiq<iimax1)dm=dist2(coordtot[i][iip],coordtot[j][iiq]);
                        else if(iiq<iimax1)dm=dist2(coord[i][iip-iimax],coordtot[j][iiq]);
                        else if(iip<iimax)dm=dist2(coordtot[i][iip],coord[j][iiq-iimax1]);
                        else dm=dist2(coord[i][iip-iimax],coord[j][iiq-iimax1]);
                        if(dm<d)
                        {
                            d=dm;
                            iim=iip;
                ...
                else
                {
                    ...
                    {
                        d1=dist2(coord[i][iim-iimax],coord[j][0]);
                        d2=dist2(coord[i][iim-iimax],coord[j][1]);
                        if(d1<d2)q=0;
                        else q=1;
                    }
                }
                ...
                {
                    if(iim>=iimax)q0=iim-iimax;
                    else if(iima<iimax1)
                    {
                        d1=dist2(coordtot[j][iima],coord[i][0]);
                        d2=dist2(coordtot[j][iima],coord[i][1]);
                        if(d1<d2)q0=0;
                        else q0=1;
                    }
                    else
                    {
                        d1=dist2(coord[j][iima-iimax1],coord[i][0]);
                        d2=dist2(coord[j][iima-iimax1],coord[i][1]);
                        if(d1<d2)q0=0;
                        else q0=1;
                    }
                }
                ...
                if(d<=(PiPiDmax*PiPiDmax))
                {
                    if(iim<iimax)
                        ...
                        for(k=0;k<3;k++)tt[k]=coord[i][q0][k]-coord[j][q][k];
                    for(k=0;k<3;k++)ttN[k]=coordN[j][q][k];
                    ...
                    if(iima<iimax1)
                        for(k=0;k<3;k++)tt[k]=coordtot[j][iima][k]-coord[i][q0][k];
                    else
                        for(k=0;k<3;k++)tt[k]=coord[j][q][k]-coord[i][q0][k];
                    for(k=0;k<3;k++)ttN1[k]=coordN[i][q0][k];
                    
                    d1=dist2(tt,ttN1);
                    for(k=0;k<3;k++)tt[k]=(-tt[k]);
                    d2=dist2(tt,ttN1);
                    if(d2>d1)for(k=0;k<3;k++)tt[k]=(-tt[k]);
                    a1=angle(tt,ttN1,true);
                    
                    if(iim>=iimax)anglemaxaro(d,&amax1,coordtot[i][4],coord[i][q0],coordN[i][q0],CATradcyl);
                    else anglemaxaro(d,&amax1,coordtot[i][iim],coord[i][q0],coordN[i][q0],CATradcyl);
                    ...
                    {
                        d1=dist2(ttN1,ttN);
                        for(k=0;k<3;k++)ttN1[k]=(-ttN1[k]);
                        d2=dist2(ttN1,ttN);
                        if(d2>d1)for(k=0;k<3;k++)ttN1[k]=(-ttN1[k]);
                        a1=angle(ttN,ttN1,true);
                        a1=a1*180.0/M_PI;
                        ...
                        if(iim>=iimax&&seq[i]=='H')
                            strcpy(aux3,"mC5");
                        else if(iim>=iimax&&q0==0)
                            strcpy(aux3,"mC6");
                        else if(iim>=iimax&&q0==1)
                            strcpy(aux3,"mC5");
                        else
